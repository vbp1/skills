...
name: sparring
description: |
  Debate a plan, an idea, an architecture or a risky change with Codex CLI as the
  ...
  alternatives.
  Triggers (RU): "спарринг", "спарринг с codex", "поспорь с codex", "обсуди с codex",
  "второе мнение", "второе мнение от codex", "оспорь план с codex".
  Triggers (EN): "sparring", "spar with codex", "second opinion", "second opinion from
  codex", "discuss this with codex", "challenge this plan with codex".
---

# Sparring

The opponent is Codex CLI, always. Never open a second Claude for sparring; a debate
with the same model returns the same blind spots.
...
| Path | Holds |
|---|---|
| `/tmp/<slug>.prompt` | the prompt for the current turn, written with the Write tool |
| `/tmp/<slug>.session` | the Codex session; reuse it for every turn of this debate |
...
Write the prompt to `/tmp/<slug>.prompt` with the Write tool, then pick one of the two
forms below. Both write the answer to `--out` and print `wrote answer: ...` when done.

**Short turn** — a focused rebuttal, a single clarifying question, anything expected
under nine minutes. Run inline, with the Bash tool `timeout` parameter set to `600000`:
...
that asks the opponent to check freshness against the web. Run detached, with the Bash
tool `run_in_background` left `false`:
...
If codex dies with a sandbox or system-API error, rerun the same command with
...
5. Next steps.
6. One line naming the opponent (Codex) and the number of turns.
